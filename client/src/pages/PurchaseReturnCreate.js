...
export default function PurchaseReturnCreate() {
  ...
  const [form, setForm] = useState({ supplier: '', purchaseInvoice: '', returnDate: new Date().toISOString().split('T')[0], reason: 'expired', notes: '', debitNoteNo: '' });
  const [items, setItems] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  useEffect(() => {
    API.get('/suppliers', { params: { limit: 200 } }).then(r => { if (r.success) setSuppliers(r.data); });
    API.get('/purchase-invoices', { params: { limit: 200 } }).then(r => { if (r.success) setPurchaseInvoices(r.data); });
  }, []);

  const handleInvoiceSelect = async (invoiceId) => {
    if (!invoiceId) { setItems([]); return; }
    setForm({ ...form, purchaseInvoice: invoiceId });
    try {
      const res = await API.get(`/purchase-invoices/${invoiceId}`);
      if (res.success) {
        setForm(prev => ({ ...prev, supplier: res.data.supplier?._id || prev.supplier }));
        setItems(res.data.batches.map(b => ({
          medicine: b.medicine,
  ...
  const updateItem = (idx, field, value) => {
    ...
    if (field === 'qty') {
      ...
      updated[idx].gstAmount = ((value || 0) * (updated[idx].rate || 0) * (updated[idx].gstRate || 5)) / 100;
    }
    setItems(updated);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success) navigate('/purchase-returns');
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5">
      <PageHeader icon="undo" title="New Purchase Return (Debit Note)" subtitle="Return goods to a supplier" />
      <GlassCard>
