        ...
        {loading ? <div className="flex justify-center py-8"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div></div> : data && (
          <div className="space-y-6">
            <div className="grid grid-cols-3 gap-4">
              <div className="p-4 bg-blue-50 rounded-lg text-center"><p className="text-2xl font-bold">{data.summary.totalInvoices}</p><p className="text-sm text-gray-500">Total Invoices</p></div>
              <div className="p-4 bg-purple-50 rounded-lg text-center"><p className="text-2xl font-bold">{data.b2b.length}</p><p className="text-sm text-gray-500">B2B</p></div>
              <div className="p-4 bg-green-50 rounded-lg text-center"><p className="text-2xl font-bold">{data.b2c.length}</p><p className="text-sm text-gray-500">B2C</p></div>
            </div>
            <div className="text-right text-lg font-bold">Total Sales: ₹{data.summary.totalSales?.toFixed(2)}</div>
